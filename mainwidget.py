...
from popups import ModbusPopup, ScanPopup, MotorPopup, TemperaturaPopup, CorrentePopup, TensaoPopup, CommandsPopup, RealTimeGraphPopup, HistGraphPopup
...
class MainWidget(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self._temp = TemperaturaPopup()
        self._corr = CorrentePopup()
        self._ddp = TensaoPopup()
    ...
    def updateGUI(self):
        ...
        for key, value in self._tags.items():
            ...
            if value['type'] == "motorInfo" or value['type'] ==  "negative":
                self._motor.ids[key].text = str(self._meas['values'][key]) + " " + value['unit']
            
            elif value['type'] == "TempMotor":
                self._temp.ids[key].text = str(self._meas['values'][key]) + " " + value['unit']

            elif value['type'] == "CurrentMotor":
                self._corr.ids[key].text = str(self._meas['values'][key]) + " " + value['unit']

            elif value['type'] == "DDPMotor":
                self._ddp.ids[key].text = str(self._meas['values'][key]) + " " + value['unit']
                ...
                if self._meas['values'][key] == 0:
                    ...
                    Clock.schedule_once(partial(self.changeBackgroundColor, 'img_mari', "imgs/Mari_ligado.png"))
                elif self._meas['values'][key] == 1:
                    ...
                    Clock.schedule_once(partial(self.changeBackgroundColor, 'img_mari', "imgs/Mari_desligado.png"))
                ...
                if self._meas['values'][key] == 0: 
                    ...
                    self._commands.ids['mv_escreve'].disabled = False

                elif self._meas['values'][key] == 1:
                    self._commands.ids['manual'].active = True
                    self._commands.ids['mv_escreve'].disabled = True
  
        #Atualização do gráfico
        self._realTimeGraph.ids.real_timeGraphPit.updateGraph((self._meas['timestamp'],self._meas['values']['freq_rot']),0)
        self._realTimeGraph.ids.real_timeGraphFit.updateGraph((self._meas['timestamp'],self._meas['values']['vel_est']),0)
    ...
    def changeBackgroundColor(self, widgetId, source, dt):
        """
        Método para alterar a cor de fundo de uma label
        """
        self.ids[widgetId].source = source
